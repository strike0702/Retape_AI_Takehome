...
Full problem statement: `[ASSIGNMENT.md](./ASSIGNMENT.md)`.
...
4. Rank the feasible ones: fee timing first, then the client-protection
...
| ----------------------- | --------------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| `even_pays`             | Exact even split of the offer total; remainder cents on the latest payments | `"even"`                                                                      |
...
non-decreasing. Example: `S = 10003`, `k = 4` → base `2500`, remainder `3` →
`[2500, 2501, 2501, 2501]`.
...
the prior payment and its own floor. Example: floors
`[2500, 2500, 2500, 2500]`, `S = 17500`, `k = 4` →
...
base `2500`, `max_token_pays = 2`, no tiers, `k = 4` → floors
`[2500, 2500, 2501, 2501]`.
...
required step-up above `v_a`. Example: `k = 4`, `max_segments = 2`, floors all
`2500`, `S = 12000`, groups of sizes `(2, 2)`. Then `R = 12000`, and one
solution is `v_a = 2500`, `v_b = 3500` → `[2500, 2500, 3500, 3500]`.

I also always emit a nearly flat (quasi-even) schedule — same remainder rule as
even pays. The reason is practical: the most extreme balloon can leave too
little cash in the final window to collect the fee (`w_M < F`), while a flatter
schedule would still work. Skipping that extreme would call some offers
...
is: whenever the fee objective does not force a choice, pick the schedule that
leaves the client with more cash resilience and less concentration risk —
...
3. Prefer a smaller cliff ratio `p_k / median(p)`, where `p_1, …, p_k` are the
  creditor payment amounts in cents and `median(p)` is their lower median —
   so we are not needlessly concentrating the settlement on one huge final hit.
...
There is also an optional soft safety cushion: a two-pass search that first
prefers candidates keeping a configured buffer above zero, then falls back to
buffer zero if none exist. I left the defaults at zero
...
- **No “raise early to merge tiers” search.** If `max_segments` is tight and
several tier/token floor steps fall inside `k`, one way to stay legal is to
pay *above* an early floor so two steps collapse into one segment and the
prefix can run longer. I do not search that band. Paying above floor early
usually loses on fee timing, so it only helps in narrow creditor setups.
- **Soft cushion defaults off.** The optional buffer above zero
(`DEFAULT_BUFFER_CENTS` / `BUFFER_DRAFT_PCT` in `constants.py`) delays fee 
collection whenever it binds. I keep both at zero for assignment faithful runs; 
turning them on is an explicit product choice.
